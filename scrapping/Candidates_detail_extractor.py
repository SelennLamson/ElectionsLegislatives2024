...
for ligne in div_table.find_all('div', class_='tableauLigne'):
    ...
    for i, candidat_element in enumerate(candidat_elements):
        ...
        candidat_nom = candidat_element.get_text().strip().replace('\u00a0', ' ').replace("\n",'')
        desist = candidat_element.find('span', class_='desistement') is not None
        if desist:
...
with open('candidates_details.json', 'w', encoding='utf-8') as json_file:
